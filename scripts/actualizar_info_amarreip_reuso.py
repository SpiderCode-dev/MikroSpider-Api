...
##FUNCION PARA ACTUALIZAR INFORMACION DE CLIENTES CON AMARRE DE IP MAC SIN DHCP Y PLAN DE REUSO
...
def crearclientes_amarreipmac_reuso(ip_host, user, pwd, port, nombres, ips, macaddress, interfaz, plan):
    try: # Estructura try para controlar el error si no se establece la conexión
        ...
        lista_ids = []
        for ip in ips:
            # Se elimina la informacion antigua del cliente
            ...
            # Se añade la informacion nueva al MKT
            num_cola = cola.get()
            if len(num_cola) == 0:
                lat = str(plan[i]) + "M"
                mli = str(plan[i]) + "M"
