...
##FUNCION PARA ACTUALIZAR INFO DE CLIENTES CON IP STATICA SIN AMARRE Y REUSO
...
def crearclientes_ipstatic_reuso(ip_host, user, pwd, port, nombres, ips, plan):
    try: # Estructura try para controlar el error si no se establece la conexión
        ...
        for ip in ips:
            ...
            parent2rm_queue = item_queue['parent']
            address_list.remove(id=id2rm_addlist)
            cola.remove(id=id2rm_queue)
            delete_queue_parent = cola.get(name=parent2rm_queue)
            item_queue_parent = delete_queue_parent[0]
            id_queue_parent = item_queue_parent['id']
            target2rm = item_queue_parent['target'].split(sep=",")
            target2rm.remove(ip+"/32")
            new_target = ",".join(target2rm)
            cola.set(id=id_queue_parent, targer=new_target)
            # Se añade la nueva información del cliente
            address_list.add(address=ip, comment=nombres[i], list="ips_autorizadas_mikrospider") #Agrega el cliente en el address_list
            num_cola = cola.get() # Obtiene la info de todas las colas creadas
            if len(num_cola) == 0:
                lat = str(plan[i]) + "M"
                mli = str(plan[i]) + "M"
