...
##FUNCION PARA CREAR CLIENTES IP DINAMICA PPPoE + QUEUE ESTÁTICAS + ARP MARK STATIC
...
def crearclientes_dhcp_pppoe(ip_host, user, pwd, port, nombres, ips, macaddress, interfaz, plan, localip, user_pppoe, pwd_pppoe, perfil_pppoe):
    try: # Estructura try para controlar el error si no se establece la conexión
        ...
        for ip in ips:
            ...
            ppp.add(caller_id=macaddress[i], comment=nombres[i], local_address=localip, name=user_pppoe, password=pwd_pppoe,
                    profile=perfil_pppoe, service="pppoe")
